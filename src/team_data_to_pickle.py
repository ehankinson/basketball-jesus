...
def process_team_data(reading_file: list, team_stats: dict, year: str):
    ...
    for line in reading_file:
        ...
        if len(team_stats[team]) <= end_game:
            ...
            if int(year[:4]) < 2002 and playoff_round[playoff_teams[team]['round_count']] == 'round1':
                end_of_round = 3
            else:
                end_of_round = 4
            if playoff_teams[team]['wins'] == end_of_round or playoff_teams[team]['losses'] == end_of_round:
...
def make_00_pickle(year: int):
    with open(f"{year} NBA Team Stats.pickle", "rb") as pk:
        league_stats = pickle.load(pk)
    with open(f"./data/pickle/{year} NBA Team Stats 00.pickle", "wb") as pk:
        pickle.dump(league_stats, pk)

def make_01_pickle(year: int):
    with open(f"./data/pickle/{year} NBA Team Stats 00.pickle", "rb") as f:
        ...
        if start_game not in league_data[new_team]:
            ...
            for game in range(1, end_game):
                ...
                if game >= start_game:
                    ...
                    new_league_data[team][game]['opp'] = league_data[team][game]['opp']
                    new_league_data[team][game]['game_type'] = league_data[team][game]['game_type']
    with open(f"./data/pickle/{year} NBA Team Stats 01.pickle", "wb") as pk:
...
def make_10_pickle(year: int):
    ...
    for team in league_data:
        ...
        for game in league_data[team]:
            ...
            new_league_data[team][game]['opp'] = league_data[team][game]['opp']
            new_league_data[team][game]['game_type'] = league_data[team][game]['game_type']
    with open(f"./data/pickle/{year} NBA Team Stats 10.pickle", "wb") as pk:
        pickle.dump(new_league_data, pk)

def make_11_pickle(year: int):
    with open(f"./data/pickle/{year} NBA Team Stats 01.pickle", "rb") as f:
        league_data = pickle.load(f)
    new_league_data = {}
    for team in league_data:
        new_league_data[team] = {}
        for game in league_data[team]:
            ...
            new_league_data[team][game]['opp'] = league_data[team][game]['opp']
            new_league_data[team][game]['game_type'] = league_data[team][game]['game_type']
    with open(f"./data/pickle/{year} NBA Team Stats 11.pickle", "wb") as pk:
        pickle.dump(new_league_data, pk)

def make_20_pickle(year: int, teams: list[str]):
    ...
    if int(year[:4]) == 2020:
        ...
        end_game = 82
    ranked_teams = srs_ranking(teams, league_data, end_game, 1)
    ranked_teams = [team[0] for team in ranked_teams]
    with open(f"data/json/standings/{year}.json") as j:
        standings = json.load(j)
    ...
    for team in league_data:
        ...
        for game in range(1, end_game + 1):
            ...
            if tm_index < opp_index:
                ...
                new_league_data[team][game]['defense'] = new_def
                new_league_data[team][game]['opp'] = league_data[team][game]['opp']
    new_east_rank = rank_teams(east_rank, 'conf', new_league_data, end_game, 1)
    new_west_rank = rank_teams(west_rank, 'conf', new_league_data, end_game, 1)
    playoff_games = {}
    for team in league_data:
        if team == 'SAS':
            a = 5
    ...
    for team in teams:
        ...
        for key in sorted_game_dict:
            if sorted_game_dict[key] == None:
                break
            count = 0
            if int(year[:4]) < 2002 and key == 'round1':
                round_end = 3
            else:
                round_end = 4
            while count < round_end:
                ...
                count += 1            
    with open(f"./data/pickle/{year} NBA Team Stats 20.pickle", "wb") as pk:
        pickle.dump(new_league_data, pk)

def make_21_pickle(year: int, teams: list[str]):
    with open(f"./data/pickle/{year} NBA Team Stats 11.pickle", "rb") as f:
        league_data = pickle.load(f)
    if int(year[:4]) == 2020:
        end_game = 72
    else:
    ...
    for team in teams:
        ...
        for key in sorted_game_dict:
            ...
            while count < round_end:
                ...
                count += 1            
    with open(f"./data/pickle/{year} NBA Team Stats 21.pickle", "wb") as pk:
        pickle.dump(new_league_data, pk)
...
if __name__ == '__main__':
    ...
    for year in range(start_year, end_year + 1):
        ...
        with open(team_file, 'r') as team_file:
            ...
            process_team_data(reading_file, team_stats, file_year)
        with open(f"{file_year} NBA Team Stats.pickle", "wb") as pk:
            pickle.dump(team_stats, pk)
        for league_type in league_types:
            match league_type:
                case '0.0':
                    make_00_pickle(file_year)
                case '0.1':
                    make_01_pickle(file_year)
                case '1.0':
                    make_10_pickle(file_year)
                case '1.1':
                    make_11_pickle(file_year)
                case '2.0':
                    make_20_pickle(file_year, teams)
                case '2.1':
                    make_21_pickle(file_year, teams)
